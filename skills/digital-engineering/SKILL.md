...
Everything else follows. If documents remain authoritative and the model is a picture of them, you have modelling, not digital engineering, and you pay the cost of both.
...
Three properties make it real, and the third is the one programmes skip:
...
The honest test: when the model and a document disagree, which does the programme believe? If the answer is "it depends who you ask", there is no ASoT yet.
...
Decide first what decisions the model must support — requirements allocation, interface definition, behaviour analysis, verification traceability, cost or performance trades. Model to that depth and no further.
...
| Behaviour | Activity, sequence, state machine, use case | What does it do, in what order, in what states? |
...
- **Curation** — someone owns model health as a job, not as a residual duty. This is the role programmes underfund and then wonder why the model rotted.
...
It earns its cost when there is a decision to make repeatedly against a specific instance: predicting maintenance for this airframe, tuning this deployed configuration, diagnosing this unit's behaviour. It does not earn its cost as a visualisation, or as a model somebody called a twin because the term appeared in a solicitation.
