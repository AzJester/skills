...
description: Handle classified work as a cleared contractor. Use when pursuing or holding a facility clearance, reading or writing a DD Form 254, sponsoring and tracking personnel clearances, planning classified storage or a secure area, running an insider threat program, handling foreign ownership or influence, or reporting a security incident. Covers facility and personnel security; export-control-and-markings covers marking and export of controlled information.
---

# Industrial security

`export-control-and-markings` covers what a document is marked and who may receive it. This covers the facility, the people and the program — the machinery that makes classified work possible at all, and the constraints it puts on schedules and staffing.

Two things make this a program concern rather than a security-office concern: **clearance lead times drive schedules**, and **a security incident can affect the organization's ability to hold classified contracts**. Both are consequences an engineering leader has to plan around.
...
A facility clearance establishes that an organization is eligible to access classified information at a given level. Three things about it that surprise people:
...
- **Foreign ownership, control or influence must be resolved.** Ownership structure, foreign investment, foreign board members and foreign business relationships are examined, and where FOCI exists it must be mitigated through an approved arrangement before or alongside the clearance. This is a corporate matter with long lead times, and it is a live issue for any organization with foreign investors or parent companies.

**The facility security officer is a required, cleared appointment**, and the role carries real personal responsibility. Insider threat program responsibility is a related required appointment.

## Step 2: Personnel clearances are the schedule risk

**Plan clearances as a critical path item, because on classified programs they usually are.** The sequence is sponsorship, submission of the security questionnaire, investigation, adjudication, then eligibility — and each stage has queues that do not compress for program urgency.
...
- **A clearance is not a badge to a specific program.** Eligibility plus a need to know plus program access are three separate things, and access to a specific compartment or program may take substantial additional time after eligibility exists.
...
- **Continuous vetting means clearances can be affected mid-program.** Financial difficulty, foreign contacts and unreported changes can all trigger review. This is a real staffing risk to carry in the register — see `risk-management`.
...
- **The classification level** authorized, and for what — access, generation, storage.
...
- **Special access requirements**, compartments, or additional program-specific controls.
...
**Read the DD 254 during the pursuit, not at award.** It determines whether you can perform at all, and what infrastructure you would need. A program discovering at kickoff that it requires storage the facility does not have is discovering a multi-month problem.
...
- **Classified processing on information systems is separately authorized.** A cleared facility does not imply an accredited system; that is `rmf-ato` work with additional constraints.
...
**A cleared contractor runs an insider threat program.** Its purpose is to detect and address concerning behavior before it becomes a compromise, and it involves gathering and reviewing indicators across security, human resources and information systems.

**Reporting obligations are broad and personal.** Cleared individuals are required to report a range of matters — foreign contacts and travel, financial problems, arrests, changes in personal circumstance, and concerning behavior by others. The obligation to report on colleagues is uncomfortable and is nonetheless a condition of holding a clearance.
...
- **Self-reporting a mistake is nearly always better than being found out.** The consequences of an unreported incident are consistently worse, for the individual and the organization.
- **A pattern of incidents is an organizational finding**, not a series of individual ones, and it can affect the facility clearance itself.
...
- **Classification decisions come from a guide, not from judgment.** Where the guide is unclear, the answer comes from the government security authority. Guessing upward wastes money; guessing downward is a security violation.
...
- **The program's security requirements belong in the estimate.** Cleared labor, facility costs, and the productivity effect of a secure environment all cost money — see `cost-estimating-and-boe`, where security is one of the most commonly omitted elements.
...
| Facility clearance assumed sufficient | No storage or accredited system | Confirm what is actually authorized |
| FOCI unexamined | Clearance blocked by ownership structure | Resolve early; it is a corporate lead-time item |
| Security cost omitted from the estimate | Overrun from cleared labor and facilities | Price cleared labor, facilities, productivity |
...
The honest one is the first, and it is the one that most reliably costs money: schedules on classified programs are frequently built as if the people are already cleared, and they are not.
