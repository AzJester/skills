...
description: Run iterative delivery inside a contract that was written for waterfall. Use when structuring an agile programme under EVM, mapping sprints to control accounts and work packages, writing a contract or CDRL set that permits iteration, aligning sprint cadence with reporting and review cadence, establishing a government product owner, or diagnosing why an "agile" programme is delivering like a waterfall one. Covers the contract layer; the software workflow skills cover a single team's practice.
---
...
The failure this exists to prevent is a programme that adopts agile ceremony and keeps waterfall commitments, paying the cost of both.
...
| **Fixed capacity** | Scope, within a funded team over a period | The customer will prioritise a backlog and accept what fits |
| **Capability-based** | The features inside a capability | Capabilities are contracted; stories are worked underneath |
| **Cost-reimbursable with a prioritised backlog** | Scope, continuously | The customer is engaged enough to prioritise |
...
**The software acquisition pathway exists for this.** DoD has an acquisition pathway designed around iterative software delivery, with value assessments and user agreements in place of traditional milestone structures. Where it applies, it removes much of this friction — check whether the programme is on it, because it changes what is possible.
...
**The hardest part is organisational, not technical.** Iterative delivery needs someone on the customer side who can prioritise, accept, and decide — available at the cadence the team works at, not at the cadence of a monthly review.
...
- **When the customer cannot supply this**, say so early and structure around it — longer increments, a proxy product owner with written delegation, or a different contract shape. Pretending it will resolve itself is how a programme discovers in month six that nothing has been accepted.
...
A programme runs several clocks at once and they need deliberate alignment.
...
| Programme review | Quarterly | `technical-reviews`, customer governance |
...
**Demonstrate working software at the increment**, to the actual users. It is the most useful thing a government programme review can contain and it is frequently replaced with slides.
...
The honest one is the first, and the test is a single question: what happened the last time the team wanted to drop something from the backlog? If the answer is that they could not, the programme is not agile regardless of its cadence.
