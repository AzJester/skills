...
description: Stand up a program after award. Use when planning transition-in or phase-in, working the first ninety days of a new contract, extracting requirements and deliverables from the awarded contract, staffing against clearance lead times, receiving government-furnished property or information, establishing the performance measurement baseline, preparing for an integrated baseline review, or taking over from an incumbent. `resource-and-capacity-management` covers staffing across several programs at once.
---
...
The failure this exists to prevent is a program that spends its first quarter discovering what it agreed to. Every hour of that comes out of the schedule, and it comes out at the point where there is least margin to give.
...
**Clearance lead time is usually the critical path, and it is usually discovered late.** Investigation and adjudication run on timescales that do not compress for program urgency. Start every clearance action immediately, including for people whose start date is months away — see `industrial-security`.
...
- **Name the accountable people early** — control account managers, technical leads, the person who owns each CDRL. Ambiguity here persists for the life of the program.
...
- **Agree the communication protocol**, including who may direct work. Informal direction from someone without authority is how unfunded scope enters a program, and it is far easier to prevent in week one than to unwind in month six.
...
- **Report honestly from the first report.** A program that reports green until it cannot has spent the credibility it needs when it has a real problem.
...
**By the end of it, the program should be boring.** Startup is finished when nobody is discovering obligations any more.
...
The honest one: nearly every problem a program has in its second year is traceable to something nobody had time to do properly in its first month.
