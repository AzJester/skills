...
description: Match the people you have to the work you have committed to. Use when planning staffing across several programmes, assessing whether the organisation can deliver what it is bidding, managing a matrix where people report two ways, setting utilisation targets, planning hiring against need dates and clearance lead times, or diagnosing why every programme is short-staffed while utilisation looks high.
---

# Resource and capacity management

`program-startup` staffs one programme. `structured-interviewing` hires one person. This is the layer above both: whether the organisation can actually supply what it has committed to, across everything running and everything being bid at once.

The failure this exists to prevent is the staffing profile that was credible in each proposal separately and is impossible in aggregate — which is discovered after the wins, when three programmes need the same six cleared engineers in the same quarter.
...
**Weight the pipeline honestly or the plan is decoration.** An organisation planning as though it will win everything is over-committed; one planning as though it will win nothing has no people ready when it does. The weighted view is the only useful one, and it depends on pWin assessments that are scored rather than felt.

**Bid and proposal effort is real capacity consumption**, and it consistently comes from the same senior people the programmes need. Planning it as though it is free is one of the most common causes of programme staffing shortfalls.

## Step 3: Understand why high utilisation makes everything late

The counter-intuitive result that most capacity problems come down to.

**As utilisation approaches 100%, waiting time rises sharply rather than smoothly.** A system loaded near its capacity has no absorptive slack, so any variation — an illness, a slipped dependency, a surprise — propagates instead of being absorbed. This is a property of loaded systems generally, and it is why an organisation at 98% planned utilisation delivers later than one at 85%, despite doing less work on paper.

Three consequences worth acting on:

**Target utilisation below full.** The right number depends on how variable the work is, but planning every person to full allocation guarantees that nothing can absorb a surprise, and surprises are certain.

**Multitasking is not free.** Splitting a person across three programmes does not give each a third of them; context switching and coordination take a real cut, and each programme experiences them as unreliable. Fewer, longer allocations deliver more.
...
Where people report functionally and are assigned to programmes, two authorities exist and the ambiguity is the point of friction.

- **Decide who allocates and who directs.** Typically the functional manager owns who goes where and their development; the programme directs the work. Write it down — undefined, it is settled repeatedly by whoever escalates hardest.
- **One allocation decision-maker across programmes.** Peer negotiation between programme managers for the same scarce person produces the loudest winning, not the best outcome.
...
- **Count the ramp.** New people consume the experienced ones for a period before contributing. On a late programme, that is a cost before it is a benefit — see `program-recovery`.
...
- **Feed real ramp rates and availability into estimates**, not idealised ones — see `cost-estimating-and-boe`, where an unachievable staffing profile is a cost estimate defect as much as a resourcing one.
...
| Planning at full utilisation | Everything late despite high utilisation | Target below full; keep absorptive slack |
...
| Allocation by peer negotiation | Loudest programme wins | One decision-maker, fast escalation |
| Hiring inside the lead time | The gap persists anyway | Work backwards from need dates; otherwise subcontract or descope |
| Aggregate never modelled | Three wins, six engineers | Model winning everything bid |
