# TEMP outline

Adapt to the program and its oversight. The test that matters: could someone resource and run the program's T&E from this document, and would they measure the right things?

## 1. Introduction

**Mission description** — what the system does, for whom, in what operational context. Every evaluation judgment traces back here.

**System description** — configuration, key interfaces, what is government-furnished, what is inherited.

**System threat assessment** — the validated threat the system must operate against. OT&E realism is judged against this.

## 2. Test program management and schedule

Who runs what. Program, operational test agency, oversight. Integration with the acquisition schedule and with `technical-reviews` gates.

Name the decision each test event informs. An event that informs no decision is one to question.

## 3. Evaluation framework

The section that determines whether the T&E program is worth running.

- **Critical operational issues** — the small number of questions that decide whether the system should be fielded.
- **MOEs, MOSs, MOPs** beneath each COI — from `measures-of-effectiveness`.
- **Thresholds** for each measure, and where each came from.
- **Data requirements** — what must be collected to evaluate each measure, at what fidelity, and how many trials.

Test the framework backwards before agreeing it: for each measure, name the event that produces its data and the resource that makes that event possible. A measure with no event behind it will not be evaluated, whatever the plan says.

## 4. Developmental test and evaluation

Events, objectives, entrance and exit criteria, articles, instrumentation, and which requirements each event verifies. Cross-reference the VCRM from `verification-validation` rather than restating it.

## 5. Operational test and evaluation

Events, operational realism, unit and operator selection, training to fielded standard, threat representation, scenarios, and data collection.

State explicitly what will be less than fully realistic and why. Every OT has limitations; naming them is credibility, and having them found is not.

## 6. Cybersecurity T&E

The six phases, when each occurs, who runs it, and what each produces. Link findings into the `rmf-ato` package as assessment evidence.

## 7. Modeling and simulation

Which models substitute for which live testing, their accreditation status, the intended use each is accredited for, and their stated limitations.

## 8. Resource summary

Ranges, facilities, threat systems, instrumentation, test articles, spares, people, funding.

The most common way a TEMP becomes fiction. Reconcile this against section 3 before agreeing the document — if the framework needs a resource this section does not fund, one of the two is wrong, and it is better to find that now.
