...
description: Design the system around the people who will operate and maintain it. Use when writing human performance requirements, analyzing operator workload or task allocation, deciding what a human versus the system should do, planning manpower and training implications of a design, assessing human error potential, or arguing that a design is unusable by the people who will actually be given it. Covers the HSI domains as a program discipline.
---

# Human systems integration

The discipline that asks whether the people who will operate, maintain and support the system can actually do so — at the manning level the program assumed, with the training the program funded, under the conditions the mission imposes.
...
**The trades between them are the whole point.** Automation reduces manpower and raises training and skill requirements. Simplifying the interface reduces training and may raise development cost. Reducing crew size raises workload on those remaining and can affect survivability. Optimizing any one domain in isolation moves cost into another, usually into manpower — which is the most expensive and the one the program office does not pay for.
...
HSI influence decays sharply with program maturity. In concept and requirements it shapes the architecture; at design review it can change an interface; at test it can only document that the system is hard to use.
...
## Step 4: Analyze the work, with methods that produce evidence
...
- **Design for the maintainer as well as the operator.** Maintenance tasks are frequently harder, performed in worse conditions, and designed last. `reliability-and-sustainment` covers the access, isolation and standardization decisions that decide whether maintenance is feasible.
...
| Domains optimized separately | Cost moves into manpower | Trade them together, explicitly |
| Automation by default | Bored monitor, bad takeover | Allocate function deliberately; design the handover |
| Nominal-case allocation | Impossible workload when degraded | Analyze the degraded case |
...
| Maintainer considered last | Maintenance infeasible in the field | Analyze maintenance tasks during design |
