...
description: Run a product rather than a program. Use when defining a product from market need rather than a customer SOW, setting unit cost targets and amortising development, deciding SKU and configuration strategy, prioritizing a product backlog against demand from several customers, planning a technology refresh, or deciding whether to build a product at all versus bid the work. Distinct from program-startup and the contract skills, which assume a customer who specified the work.
---
...
That inversion changes the economics, the decision rights and the failure modes. Running a product like a program produces something built to one customer's specification and sold once — which is a contract with extra steps and worse margin.
...
**The honest question early: is this a product, or a contract you are hoping to repeat?** A "product" with one customer, built to their requirements, funded by their program, is a contract. That may be the right business — but calling it a product leads to investing in configurability, documentation and support that a single-customer contract does not repay.
...
- **Talk to users who are not your sponsor.** The most common product failure in a contracting organization is building a generalized version of the last program's deliverable, which turns out to fit only that program.
...
- **Amortize development honestly.** Non-recurring cost recovered across a volume forecast you believe, not one that makes the business case work. State the break-even volume explicitly — it is the number that determines whether this is a business.
- **Price against value and the alternative**, not cost plus a margin. On a defense product the alternative is frequently a custom development program, which is expensive and slow — that comparison is where the value sits.
...
- **Price the special.** Where a customer genuinely needs a variant, it is non-recurring engineering they pay for, plus a decision about whether it joins the catalog.

## Step 5: Run a roadmap you can defend

- **Prioritize against demand across customers**, not by whoever asked most recently or most loudly. A named opportunity with a date beats an enthusiastic request.
...
| Generalizing the last deliverable | Fits only the original program | Define from demand across customers |
...
| Roadmap by loudest request | No coherent product, no discriminator | Prioritize against demand with dates |
